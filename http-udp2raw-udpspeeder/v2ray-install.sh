...
function install_v2ray(){
    ...
    bash <(curl https://raw.githubusercontent.com/1715173329/v2ray-easy-to-use/master/http-udp2raw-udpspeeder/install-release.sh)
    rm "/etc/v2ray/config.json" -rf 
    wget -qO /etc/v2ray/config.json "https://raw.githubusercontent.com/1715173329/v2ray-easy-to-use/master/http-udp2raw-udpspeeder/config.json" 
    ...
    nohup /root/udp2raw/udp2raw_amd64 -s -l:8080 -r127.0.0.1:10001 -a -k "${udp2raw_pwd}" --raw-mode faketcp > /dev/null 2>&1 &
    ...
    echo "nohup /root/udp2raw/udp2raw_amd64 -s -l:8080 -r127.0.0.1:10001 -a -k "${udp2raw_pwd}" --raw-mode faketcp > /dev/null 2>&1 &" >> /etc/rc.d/rc.local
    cd /root/
    clear
    echo -e "您的连接信息如下："
    echo -e "udp2raw信息："
    ...
    echo -e "地址(Address)：udpspeeder的本地监听地址"
    echo -e "端口(Port)：udpspeeder的本地监听端口"
    echo -e "用户ID(ID)：${UUID}"
